...
```js
// 落户任务状态
settleTaskStatusEnum: new Enum().add("ALL", "全部", null).add("SETTLE", "待落户", 0).add("SETTLED", "已落户", 1).add("CANCEL", "已取消", 2);
```
...
<el-select style="width: 99%;" v-model="query.taskStatus" clearable placeholder="请选择">
  ...
  <-- 基于枚举类的方法 -->
  <el-option v-for="item in enums.settleTaskStatusEnum" :key="item.value" :value="item.value" :label="item.label"></el-option>
</el-select>
```
